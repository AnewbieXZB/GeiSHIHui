...
public class SearchHistorysDao {
	...
	// 查多条记录
	public ArrayList<SearchHistorysBean> listMore() {
		...
		try {
			...
			String sql = "select * from t_historywords order by updatetime desc limit 15;";
			database = helper.getReadableDatabase();
			cursor = database.rawQuery(sql, null);
			int colims = cursor.getColumnCount();// 获得列的数目
